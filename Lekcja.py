...
if __name__ == "__main__":
    ...
    # s.check_column(x)
    # s.check_row(y)
    # s.check_nine_square(x,y)

    # s.get_column(x)
    # s.get_row(y)
    # s.get_nine_square(x,y)
    ...
    [[8, 4, None], [None, 7, 6], [9, 1, 3]]
    [8, 4, 7, 6, 9, 1, 3]
    """
    while s.check() == False:
        for y in range(9):
            for x in range(9):
                if s.solution[x][y] is None:

                    possible_solutions = list(range(1, 10))

                    row = [element for element in s.get_row(y) if element is not None]
                    column = [
                        element for element in s.get_column(x) if element is not None
                    ]
                    square = [
                        element
                        for column in s.get_nine_square(x // 3, y // 3)
                        for element in column
                        if element is not None
                    ]

                    print(f"[{x},{y}]: {s.solution[x][y]}")

                    print("possible", possible_solutions)
        ...
        for x in range(9):
            if s.solution[x][y] is None:
                ...
                row = [element for element in s.get_row(y) if element is not None]
                column = [element for element in s.get_column(x) if element is not None]
                square = [
                    element
                    for column in s.get_nine_square(x // 3, y // 3)
                    for element in column
                ...
                print("after removal:", possible_solutions)

                if len(possible_solutions) == 1:
                    s.solution[x][y] = possible_solutions[0]
